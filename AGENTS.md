...
| P-SNAP-01 | `[SNAP]` log snapshot fires only once per 10 s AND only when the per-chart TRADEPLAN sig changes — so if the chart is on a very stable TF (D1 weekend) the snapshot may not appear for a long time | The snapshot gate `s_snapMs` is inside `TradePlanLiveTick()` which itself requires `!IsIndicatorHidden()` and a 2s throttle + sig change; on a static D1 chart the sig never changes so neither does the snapshot | To force a fresh snapshot: switch the chart to a more active TF (M5/M15) or wait for the next tick/bar. `[SNAP]` lines count in `fetch_tradeplan_log.ps1` output shows if any exist; if 0, the indicator isn't ticking | 2026-09-09 |
