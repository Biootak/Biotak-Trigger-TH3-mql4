...
* چیدمان: **یک کارت پایین-راست**: `#SL / #TP1 / #TP2 / #TP3` (آبی) پایین،
  و بالای آن `Hunter SL / Eng.SL` (قرمز) و مُهر TRex (برند + اسپرد زنده؛
  بدون عنوان فارسی — آن ردیف ۲۰۲۶-۰۹-۱۴ حذف شد). هر سه سطر وسط‌چین روی یک
  محور مشترک و با فاصلهٔ یکسان `inpATRTradeLabelRowGap` (ورودی خودِ کارت،
  پیش‌فرض ۱۰ پیکسل)؛ فاصلهٔ حروف `TR`/`ex`
  اندازه‌گیری‌شده است (روی DPI بالا به هم نمی‌چسبد). ورودی
  `inpTrexStampGapRows` (پیش‌فرض ۰ = جمع‌وجور) فضای اضافه می‌دهد.
  هر سطر خاموش دیگر جای اشغال نمی‌کند: سطرهای روشن پایین می‌آیند و آبجکت سطر
  خاموش توسط همان نویسنده پاک می‌شود (P-LBL-09/10). سه ورودی مخصوص خودِ کارت:
  `inpLabelsMarginBottom` (فاصله از کف، پیش‌فرض ۸)، `inpATRTradeLabelRowGap`
  (فاصلهٔ سطرها، پیش‌فرض ۱۰) و `inpTrexStampGapRows` (سطر خالی اضافه). `Close in` (قرمز) دیگر در گوشه نیست —
  تگ جمع‌وجور کنار کندل لایو و هم‌سطح قیمت بید/اسک است و با قیمت حرکت می‌کند؛
  لایهٔ مستقلی است (کلید `D` + ردیف‌های ۰..۳ کارت ATR: خاموش/روشن، رنگ،
  اندازه، فاصله) — با خاموش کردن ATR نمی‌رود (۲۰۲۶-۰۹-۱۱).
