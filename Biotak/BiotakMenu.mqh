...
int HandleButtonClick(const string clickedObject)
{
   ...
   else if(feat == CIR_LOCK)
   {
      g_timeframeLocked = !g_timeframeLocked;
      string lockFlagName = "Biotak_LockTF_" + GetCachedChartIdStr();
      GlobalVariableSet(lockFlagName, g_timeframeLocked ? 1.0 : 0.0);
      if(g_timeframeLocked && g_lockedPeriod == 0)
      {
         g_lockedPeriod = Period();
         string lockPeriodName = "Biotak_LockTFPeriod_" + GetCachedChartIdStr();
         GlobalVariableSet(lockPeriodName, (double)g_lockedPeriod);
      }
      refreshFlags = REFRESH_ALL;
   }
   // TH3TOOL-OFF:
