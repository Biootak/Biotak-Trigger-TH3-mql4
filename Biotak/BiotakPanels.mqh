...
//--- the wide card's COMPOSED body (P-UI-71c). `pnl_cardW{n}` stops at 12
//--- pair-lines while card 2 needs 13 and the Base Box card 19, so a wide body
//--- is built from these four pieces (top cap + one band per pair-line + the
//--- footer cap + the `.fade` wash — tools/slice-card-skins.py). A piece that is
//--- sliced but NOT declared here compiles clean and renders as NO CARD AT ALL:
//--- that is exactly what shipped (the user's "پشت پس زمینه نداره"), so
//--- `tools/panel-wiring-audit.py` now fails the build when a runtime
//--- `::Files\\Icons\\*.bmp` reference has no declaration (its [chrome] check).
#resource "\\Files\\Icons\\pnl_cardWbot.bmp"
#resource "\\Files\\Icons\\pnl_cardWfade.bmp"
...
//--- so 16 is already four rows of headroom there. WIDE cards do NOT use this
//--- bound any more - their body is composed (P-UI-71b), because the bound bit
//--- them: card 2 needs 13 pair-lines and BASE BOX 19, while the baked wide
//--- skins stop at PNL_WIDE_ROWS_MAX, and the rows past that drew on the chart
//--- with no card behind them.
//--- ICON-DIET 2026-09-12: 17..20 (+f) were the 8 largest files with zero
//--- runtime path, so they are no longer emitted.
...
void PnlSpecBuild(const int item)
{
   ...
   else if(item == 2)   // ATR LABELS (gold) — 11 settings / 15 display rows
   {
      ...
      // `inpATRTradeLabelRowGap`, TRADE SIZE -> `inpATRTradeLabelFontSize`,
      // STAMP GAP -> `inpTrexStampGapRows`, CARD MARGIN ->
      // `inpLabelsMarginBottom`) and every one of them is clamped by the card's
      // own named bound, so a slider can never ask for a layout the owner
      // refuses. ROW GAP is the row P-LBL-09 retired (`g_atrLabelRowGap` was
      // dead then; the card consumes it now). The label GRID still reads
      // `inpLabelRowGap` — none of these rows may move the ATR/TH columns.
      PnlSpecAdd(2, PNL_K_SEC, -1, 0, "", "", "TRADE CARD", 4);
      PnlSpecAdd(2, PNL_K_LEGACY, 10, 1, "gap");
      PnlSpecAdd(2, PNL_K_LEGACY, 11, 1, "textsize");
      PnlSpecAdd(2, PNL_K_LEGACY, 12, 1, "valign", "", "", 0, "ROWS");
      PnlSpecAdd(2, PNL_K_LEGACY, 13, 1, "margin");
      // ...and the card's five COLOURS, folded into ONE .cset row the way the HTF
      // card folds its four (P-UI-70d). The order is the drawing order
      // (TR, ex, Hunter, #SL/TP, spread) and the setting addresses are
      // consecutive so the palette's per-cell target ring and the FIVE next
      // free addresses stay readable in the persisted override keys.
...
void PalDraw()
{
   ...
   ObjectSetInteger(0,p+"cur",OBJPROP_BORDER_COLOR,PnlSwatchBorder(cur,PNL_CLR_FIELD));
               ...
               int &minV,int &maxV,double &step,string &unit,string &opts)
{
   ...
   else if(item==6)   // HTF CANDLES
   {
      ...
      else if(row==10) { kind=2; label="BOX MODE"; opts="Hollow|Filled|Both"; }
   ...
   if(kind == PNL_K_COL)
   {
      ...
      int sy = ry + 18;
       PnlSetButton(PnlName(item,row,"CB"), px+PNL_PAD_X, sy, PNL_QSW_PREV, 22, "", cc,
                    (g_PalOpen && g_PalKind==kk) ? a1 : PnlSwatchBorder(cc,PNL_CLR_CARD), true);
...
void PnlUpdateRow(const int item,const int row)
{
   ...
   if(kind==4)   // color row: refresh preview swatch + quick-pick selection rings
   {
      ...
      if(ObjectFind(0,cb)>=0)
      {
         ...
         ObjectSetInteger(0,cb,OBJPROP_BORDER_COLOR,PnlSwatchBorder(cur,PNL_CLR_CARD));
      }
      for(int qi=0; qi<PNL_QSW_N; qi++)
      {
