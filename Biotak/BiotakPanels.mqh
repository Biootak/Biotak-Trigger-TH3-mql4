...
bool PnlIsStyleRow(const int item,const int row)
{
   ...
   if(item==12 && row==3 && g_BkTab==0) return true;   // Base Box STYLE (Style tab)
...
int PnlRowsCount(const int item)
{
   ...
   if(item == 12) return 1 + BkSecRows();           // TAB + open tab's section (TV-parity)
                 ...
                 int &minV,int &maxV,double &step,string &unit,string &opts)
{
   ...
   else   // STYLE
   {
      ...
      else if(sec==2)  { label="STYLE"; minV=0; maxV=ILS_COUNT-1; }
...
void PnlCreateRow(const int item,const int row,const int px,const int py)
{
   ...
   else if(kind==2)   // ── TAB underline / dropdown select / pills ──
   {
    ...
    {                  // text control — same as the palette hex field. Commits on
       string en=PnlName(item,row,"ED");   // ENTER (CHARTEVENT_OBJECT_ENDEDIT).
       string cur="";
